...
cd "$DIST_DIR"
tar cf - dist | sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no $SERVER '
  cd /tmp && rm -rf dist && tar xf -
  # 只复制新文件，不删除旧文件
  sudo cp -rn dist/* /var/www/jxy/ 2>/dev/null || sudo cp -r dist/* /var/www/jxy/
  echo "部署完成: $(ls /var/www/jxy/assets/*.js | wc -l) 个JS文件"
'
echo "✅ 前端部署完成"
