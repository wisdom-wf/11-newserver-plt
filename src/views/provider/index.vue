...
const form = ref({
  ...
  description: '',
  status: 'ACTIVE' as Api.Common.EnableStatus
});
...
async function getTableData() {
  ...
  try {
    ...
    if (searchName.value) params.providerName = searchName.value;
...
function handleAudit(row: Api.Provider.Provider) {
  currentAuditId.value = row.providerId;
    ...
    <NCard title="服务商统计" :bordered="false" style="margin-bottom: 16px">
      <div class="statistics-grid">
        ...
        <div class="stat-card stat-warning">
          ...
          <div class="stat-value">{{ statistics.pendingProviders }}</div>
        </div>
        <div class="stat-card stat-success">
          <div class="stat-label">已通过</div>
          <div class="stat-value">{{ statistics.approvedProviders }}</div>
        </div>
      </div>
    </NCard>
