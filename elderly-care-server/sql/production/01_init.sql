...
CREATE TABLE IF NOT EXISTS t_service_evaluation (
    ...
    provider_id VARCHAR(32) NOT NULL COMMENT '服务商ID',
    elder_id VARCHAR(32) COMMENT '老人ID',
    quality_check_id VARCHAR(32) COMMENT '关联质检单ID',
    service_log_id VARCHAR(32) COMMENT '关联服务日志ID',
    ...
    replyer_id VARCHAR(32) COMMENT '回复人ID',
    replyer_name VARCHAR(100) COMMENT '回复人姓名',
    evaluation_token VARCHAR(128) COMMENT '评价邀请Token',
    token_status VARCHAR(20) DEFAULT 'PENDING' COMMENT 'Token状态: PENDING/COMPLETED/EXPIRED',
    token_expire_time DATETIME COMMENT 'Token过期时间',
    token_used_time DATETIME COMMENT 'Token使用时间',
    token_used_ip VARCHAR(50) COMMENT '提交IP',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    deleted TINYINT DEFAULT 0 COMMENT '删除标记',
    INDEX idx_order (order_id),
    INDEX idx_staff (staff_id),
