...
echo "[2/3] 上传 JAR..."
cd "$(dirname "$JAR")"
tar cf - elderly-care-server-1.0.0.jar | sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no $SERVER '
  rm -f /opt/jxy/elderly-care-server-1.0.0.jar
  cd /opt/jxy && tar xf - && echo "JAR OK"
'

echo "[3/3] 重启后端..."
cat /Users/works/my-projects/11-newserver-plt/quick-restart.sh | sshpass -p "$PASS" ssh -o StrictHostKeyChecking=no $SERVER 'bash /opt/jxy/quick-restart.sh'
