...
-- 2. 创建服务日志
INSERT INTO service_log (log_id, log_no, order_id, order_no, elder_id, elder_name, service_type_code, service_type_name, service_date, service_start_time, service_end_time, service_duration, service_content, service_result, staff_id, staff_name, provider_id, provider_name, status, create_time, update_time, deleted) VALUES
...
-- 3. 创建质检记录
INSERT INTO quality_check (check_id, check_no, order_id, order_no, elder_id, elder_name, service_log_id, service_log_no, check_type, check_method, check_content, check_result, check_score, check_status, check_user_id, check_user_name, provider_id, provider_name, create_time, update_time, deleted) VALUES
('QC001', 'QC20260514001', 'ORDER001', 'ORD20260514001', 'ELDER001', '张大爷', 'LOG001', 'SL20260514001', 'SERVICE_QUALITY', 'PHONE', '对居家照料服务质量进行检查', 'GOOD', 85, 'COMPLETED', NULL, NULL, (SELECT provider_id FROM t_elder WHERE elder_id='ELDER001'), NULL, NOW(), NOW(), 0);
...
SELECT log_id, log_no, order_no, elder_name, service_type_name, status FROM service_log;

SELECT '质检记录详情:' as info;
SELECT check_id, check_no, order_no, elder_name, check_result, check_score FROM quality_check;